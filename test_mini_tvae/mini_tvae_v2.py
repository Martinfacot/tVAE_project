    ...
    for column_info in output_info:
        for span_info in column_info:
            if span_info.activation_fn != 'softmax':
                # Continuous
                ed = st + span_info.dim
                std = sigmas[st] # standard deviation
...
class MiniTVAE:
    ...
    def __init__(
        ...
        self.epochs = epochs
        self.loss_values = pd.DataFrame(columns=['Epoch', 'Batch', 'loss_1', 'loss_2', 'total_loss'])
    ...
    def fit(self, train_data, discrete_columns=()):
        ...
        # Initialize DataFrame to store loss values with all required columns
        self.loss_values = pd.DataFrame(columns=['Epoch', 'Batch', 'loss_1', 'loss_2', 'total_loss'])
        ...
        for i in iterator:
            ...
            for id_, data in enumerate(loader):
                ...
                total_loss = loss_1 + loss_2  # total loss is the sum of reconstruction loss and KLD loss
                total_loss.backward()
                optimizerAE.step()
                ...
                batch_losses.append({
                    'Epoch': i,
                    'Batch': id_,
                    'loss_1': loss_1.detach().cpu().item(),
                    'loss_2': loss_2.detach().cpu().item(),
                    'total_loss': total_loss.detach().cpu().item()
                })

            # Convert batch losses to DataFrame and append to loss_values
            epoch_loss_df = pd.DataFrame(batch_losses)
            ...
            if self.verbose:
                # Use the total_loss for the progress bar
                avg_total_loss = epoch_loss_df['total_loss'].mean()
                iterator.set_description(iterator_description.format(loss=avg_total_loss))
